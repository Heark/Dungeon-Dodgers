...
module GameJolt
  ...
  GameId = "202914"
  PrivateKey = "e73569b04dbe5760f32fd04d8937f881"
  ...
  def self.login()
    ...
    config.opacity = 64
    $user_name = Scene_Text.read("What is your username?", "center", 24, true, config)
    $user_name.gsub!("&", "_")
    user_token = Scene_Text.read("What's your user token?", "center", 24, true, config)
    user_token.gsub!("&", "_")
  ...
  def self.do_request(baseUrl)
    ...
    def self.get_server(key)
    urlHash = ZOMD5.calc_md5("http://gamejolt.com/api/game/v1/data-store/?game_id=" + GameId + "&key=" + key + "&format=json" + PrivateKey)
    urlHash = urlHash[0]
    result = EFE.request("gamejolt.com", "api/game/v1/data-store/?game_id=" + GameId + "&key=" + key + "&format=json&signature=" + urlHash)
    result = JSON.decode(result)
    if result != nil
  ...
  def self.userget_server(key)
    user_name = CustomData.get("gj_username")
    user_token = CustomData.get("gj_usertoken")
    urlHash = ZOMD5.calc_md5("http://gamejolt.com/api/game/v1/data-store/?game_id=" + GameId + "&key=" + key + "&username=" + user_name + "&user_token=" + user_token + "&format=json" + PrivateKey)
    urlHash = urlHash[0]
    result = EFE.request("gamejolt.com", "api/game/v1/data-store/?game_id="  + GameId + "&key=" + key + "&username=" + user_name + "&user_token=" + user_token + "&format=json&signature=" + urlHash)
    result = JSON.decode(result)
    if result != nil
      return result["response"]["data"]
    else
    ...
    puts "Data successfully gotten from server."
  end
   def self.send_server(key, data)
    urlHash = ZOMD5.calc_md5("http://gamejolt.com/api/game/v1/data-store/set/?game_id=" + GameId + "&key="+key+"&data="+data+"" + PrivateKey)
    urlHash = urlHash[0]
    ...
    user_name = CustomData.get("gj_username")
    user_token = CustomData.get("gj_usertoken")
    urlHash = ZOMD5.calc_md5("http://gamejolt.com/api/game/v1/data-store/set/?game_id=" + GameId + "&key=" + key + "&data=" + data + "&username=" + user_name + "&user_token=" + user_token + "" + PrivateKey)
    urlHash = urlHash[0]
    result = EFE.request("gamejolt.com", "api/game/v1/data-store/set/?game_id=" + GameId + "&key="+key+"&data="+data+"&username="+user_name+"&user_token="+user_token+"&signature=" + urlHash)
    puts result
    result = JSON.decode(result)
    puts result.to_s
    if result != nil
